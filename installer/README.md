...
1. 安装内置 `PetitPlanetRootCA.cer` 到「受信任的根证书颁发机构」（`certutil -addstore Root`）
...
& "${env:ProgramFiles(x86)}\Inno Setup 6\ISCC.exe" /DMyAppVersion=0.1.0 installer\PetitPlanetWikiSetup.iss
```
